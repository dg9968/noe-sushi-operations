...
import { Salad, ChefHat, Fish, Martini, IceCream, Soup } from "lucide-react";
...
const OptimizedRecipeManager: React.FC = () => {
  ...
  const [allIngredients, setAllIngredients] = useState<
    { id: string; name: string; unitCost?: number }[]
  >([]);
  const [availableIngredients, setAvailableIngredients] = useState<
    { id: string; name: string; unitCost?: number }[]
  >([]);
  const [allRecipes, setAllRecipes] = useState<
    { id: string; name: string; costPerServing?: number }[]
  >([]);
  ...
  const [showIngredientDropdowns, setShowIngredientDropdowns] = useState<{
    [key: number]: boolean;
  }>({});
  const [showRecipeMode, setShowRecipeMode] = useState<{
    [key: number]: boolean;
  }>({});
  ...
  useEffect(() => {
    const initializeServices = async () => {
      ...
      if (apiAvailable) {
        ...
        console.log(
          "⚠️ API service unavailable, falling back to direct Airtable"
        );
  ...
  useEffect(() => {
    if (Object.keys(recipesByCategory).length > 0) {
      ...
      if (recipes.length >= allRecipes.length || allRecipes.length === 0) {
        ...
        console.log(
          `📋 Skipping pre-population (search results: ${recipes.length}, cached: ${allRecipes.length})`
        );
      }
    }
  }, [recipesByCategory]);
  ...
  const loadRecipeList = async (searchQuery?: string) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load recipe list"
      );
    } finally {
  ...
  const searchRecipes = async (query: string, index: number) => {
    ...
    console.log(
      `🔍 searchRecipes called: query="${query}", allRecipes.length=${allRecipes.length}, selectedRecipe=${selectedRecipe?.name}`
    );
    console.log(
      `🔍 First 3 recipes in allRecipes:`,
      allRecipes.slice(0, 3).map((r) => r.name)
    );
    ...
    if (query.length > 0) {
      try {
        ...
        console.log(
          `🔍 Filtered to ${filtered.length} recipes matching "${query}"`
        );
        console.log(
          `🔍 Filtered recipes:`,
      ...
      const recipesExcludingCurrent = allRecipes.filter(
        (recipe) => recipe.id !== selectedRecipe?.id
      );
      console.log(
        `🔍 Showing ${recipesExcludingCurrent.length} total recipes (excluding current)`
      );
      console.log(
        `🔍 First 5 available:`,
        recipesExcludingCurrent.slice(0, 5).map((r) => r.name)
      );
  ...
  const addIngredientToRecipe = (
    ...
    const fromOdoo =
      ingredient.unitCost !== undefined && ingredient.unitCost > 0;
    ...
    newIngredients[index] = {
      ...
      totalCost:
        (newIngredients[index]?.quantity || 1) * (ingredient.unitCost || 0),
      fromOdoo: fromOdoo,
    };
  ...
  const addRecipeAsIngredient = async (
    recipe: { id: string; name: string; costPerServing?: number },
    index: number
  ) => {
    if (!selectedRecipe) return;
    ...
    try {
      ...
      if (useApiService) {
        ...
        if (fullRecipe) {
          costPerServing =
            fullRecipe.costPerServing ||
            fullRecipe.totalCostWithQFactor ||
            fullRecipe.totalCost ||
            0;
        }
      } else if (airtableService.isEnabled()) {
    ...
    <div className="recipe-manager">
      <div className="recipe-manager-header">
        ...
        <p className="recipe-manager-subtitle">
          Manage and cost your recipes with precision
        </p>
        ...
        <div className="recipe-list-panel">
          ...
          {categories.length === 0 && searchTerm && !isLoadingList && (
            <div className="no-selection">
              ...
              <p>
                No recipes match your search term "{searchTerm}". Try a
                different search.
              </p>
            </div>
          )}
            ...
            <div key={category} className="category-section">
              <h3
                ...
                className={`category-header ${
                  selectedCategory === category ? "expanded" : ""
                }`}
                style={{
                  cursor: "pointer",
                  padding: "0.75rem 1rem",
                  background:
                    selectedCategory === category ? "#e0e7ff" : "transparent",
                  borderRadius: "8px",
                  marginBottom: "0.5rem",
                  display: "flex",
                  ...
                  fontWeight: "600",
                  fontSize: "0.95rem",
                  color: selectedCategory === category ? "#4338ca" : "#1f2937",
                  transition: "all 0.2s",
                  border:
              ...
              {selectedCategory === category && (
                <ItemGroup style={{ marginBottom: "1rem" }}>
                  {recipesByCategory[category].map((recipe, index) => (
                    <React.Fragment key={recipe.id}>
                      <Item
                        variant={
                          selectedRecipeId === recipe.id ? "muted" : "default"
                        }
                        size="sm"
                      ...
                      >
                        <ItemMedia variant="icon">
                          {category === "Appetizers" ? (
                            ...
                            <ChefHat size={20} />
                          ) : category === "Sushi" || category === "Sashimi" ? (
                            <Fish size={20} />
                          ) : category === "Beverages" || category === "Bar" ? (
                            <Martini size={20} />
                          ) : category === "Desserts" ? (
                            <IceCream size={20} />
                          ) : category === "Soups" ? (
                      ...
                      {index < recipesByCategory[category].length - 1 && (
                        <ItemSeparator />
                      )}
                    </React.Fragment>
                  ))}
        ...
        <div className="recipe-details-panel">
          {isLoadingRecipe ? (
            ...
            <div className="recipe-details">
              ...
              >
                {/* Recipe Image - spans 2 columns, 2 rows */}
                <div
                  style={{
                    gridColumn: "1 / 3",
                    gridRow: "1 / 3",
                    background:
                      "linear-gradient(135deg, #acadbf 0%, #19207d 100%)",
                    borderRadius: "12px",
                    padding: "1rem",
                    overflow: "hidden",
                  }}
                ...
                {/* Cost Breakdown Card - spans full width */}
                {selectedRecipe.costBreakdown && (
                  <div
                    style={{
                      ...
                      gridRow: "3 / 4",
                      background:
                        "linear-gradient(135deg, #a7abdb 0%, #b9d4a9 100%)",
                      borderRadius: "12px",
                      padding: "1.5rem",
                      display: "flex",
                  ...
                  >
                    <div style={{ textAlign: "center" }}>
                      ...
                      >
                        $
                        {selectedRecipe.costBreakdown.baseCost?.toFixed(2) ||
                          "0.00"}
                      </div>
                    </div>
                    ...
                    <div style={{ textAlign: "center" }}>
                      ...
                      >
                        Q Factor ({selectedRecipe.qFactorPercentage || qFactor}
                        %)
                      </div>
                      <div
                        style={{
                      ...
                      >
                        $
                        {selectedRecipe.costBreakdown.qFactorAmount?.toFixed(
                          2
                        ) || "0.00"}
                      </div>
                    </div>
                    ...
                    >
                      ...
                      >
                        $
                        {selectedRecipe.costBreakdown.totalWithQFactor?.toFixed(
                          2
                        ) || "0.00"}
                      </div>
                    </div>
                    ...
                    <div style={{ textAlign: "center" }}>
                      ...
                      >
                        ${selectedRecipe.costPerServing?.toFixed(2) || "0.00"}
                      </div>
                    </div>
                  </div>
                )}
                    ...
                    {selectedRecipe.ingredients.map((ingredient, index) => (
                      ...
                      >
                        <div className="ingredient-input-group">
                          {isEditing && (
                            <div className="ingredient-mode-toggle flex gap-1">
                              <Button
                                type="button"
                                variant={
                                  !showRecipeMode[index] ? "default" : "outline"
                                }
                                size="sm"
                                onClick={() => {
                                  setShowRecipeMode((prev) => ({
                                    ...prev,
                                    [index]: false,
                                  }));
                                  setIngredientSearchTerms((prev) => ({
                          ...
                          <input
                            type="text"
                            value={
                              ingredientSearchTerms[index] !== undefined
                                ? ingredientSearchTerms[index]
                                : ingredient.name
                            }
                            onChange={(e) => {
                            ...
                            className={`ingredient-search ${
                              ingredient.isRecipe ? "recipe-ingredient" : ""
                            }`}
                          />
                              ...
                              availableIngredients.length > 0 ? (
                                availableIngredients.slice(0, 10).map((ing) => (
                                  <div
                                    ...
                                    onClick={() =>
                                      addIngredientToRecipe(ing, index)
                                    }
                                  >
                                    <span className="ingredient-name">
                                      {ing.name}
                          ...
                          onChange={(e) => {
                            ...
                            if (ingredient.isRecipe) {
                              ...
                              newIngredients[index] = {
                                ...
                                totalCost:
                                  newQuantity *
                                  (ingredient.recipeCostPerServing || 0),
                              };
                            } else {
                        ...
                        <select
                          ...
                          className={`unit-select ${
                            ingredient.isRecipe ? "recipe-unit" : ""
                          }`}
                        >
                          {ingredient.isRecipe ? (
                ...
                {selectedRecipe.costBreakdown && (
                  <div className="cost-summary">
                    ...
                    <div className="cost-grid">
                      <div>
                        Ingredients Cost: $
                        {selectedRecipe.costBreakdown.baseCost?.toFixed(4) ||
                          "0.0000"}
                      </div>
                      <div>
                        Q Factor ({selectedRecipe.qFactorPercentage || qFactor}
                      ...
                      <div>
                        Cost Per Serving: $
                        {selectedRecipe.costPerServing?.toFixed(4) || "0.0000"}
                      </div>
                    </div>
                  </div>
                )}
                          ...
                          if (isEditing && selectedRecipe) {
                            ...
                            if (selectedRecipeId && useApiService) {
                              try {
                                ...
                                console.error(
                                  "Failed to update cost calculations:",
                                  err
                                );
                              }
                            }
            ...
            <div className="no-selection">
              ...
              <p>
                Choose a category and recipe from the list on the left to see
                full details, ingredients, and cost breakdown.
              </p>
            </div>
          )}
        </div>
            ...
            <div className="dialog-content">
              ...
              <div className="dialog-actions">
                ...
                <Button
                  ...
                  variant="default"
                  disabled={
                    !newIngredient.name.trim() || newIngredient.unitCost <= 0
                  }
                >
                  Add Ingredient
                </Button>
